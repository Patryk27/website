...
\... it's `-16`, because `r0` & `r1` are 64-bit numbers, so: 2 * 64 / 8 bits per byte = 16 bytes.
...
[source, rust, highlight="6,11-16,18-26,31-40"]
----
/* ... */
...
[source, rust, highlight="3-8,12-34,38-"]
----
/* ... */
...
They are called *hyperparameters* -- and if instead of keeping them hard-coded, you made them configurable via some `struct Config`, you could then create an application that'd traverse different combinations of those hyperparameters, trying to find the most optimal ones:
