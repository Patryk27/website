...
[.source-title]
https://play.rust-lang.org/?version=stable&mode=debug&edition=2018&gist=d5d734320717f0fda4d16de1f2600764[open in playground]
...
[.source-title]
https://play.rust-lang.org/?version=nightly&mode=debug&edition=2018&gist=535648a70ce2e61f12b8c38021fddbef[open in playground]
...
[NOTE]
====
Our last implementation uses a _nightly_ feature called https://github.com/rust-lang/rust/issues/63063[`min_type_alias_impl_trait`] - it allows to use `impl Trait` in places such as associated types:
...
So yeah, I guess you could call me a `#![feature(min_type_alias_impl_trait)]`-'s fan #1.
====
...
As for the crossover itself, there are https://en.wikipedia.org/wiki/Crossover_(genetic_algorithm)#Examples[many] algorithms we could implement - usually it's best to try a few of them and see whichever plays best with given problem; for simplicity, we'll go with *uniform crossover*, which can be described with a single drawing:
...
As usually when it comes to naming, there's no single best answer; we'll continue with `::create()`, because we have to decide on _something_, but keep this example in mind next time you're scratching you head, thinking "What to name this function?".
