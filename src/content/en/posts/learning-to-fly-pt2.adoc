---
title: "Learning to Fly: Let's create a simulation in Rust! (pt 2)"
tags:
  - genetic-algorithm
  - neural-network
  - rust
...
:abbr-loc: pass:[<abbr title="lines of code">LOC</abbr>]
:abbr-no-op: pass:[<abbr title="no-operation, i.e. doing nothing">no-op</abbr>]

This is second part of the `Learning to Fly` series in which we're coding a simulation of evolution using *neural network* & *genetic algorithm*:
...
Instead of launching `cargo new shorelark`, as one usually does, we went with `mkdir` - it's because we're going to use a feature of Cargo's called https://doc.rust-lang.org/cargo/reference/workspaces.html[workspaces].
...
You might've noticed that when you do `cargo build`, it sometimes prints crate names _inline_ during the `Building` phase:
...
On the other side of the spectrum, we've got rustc - it's the actual compiler that Cargo invokes for each crate it has to built, and rustc itself is at the moment https://pingcap.com/blog/rust-huge-compilation-units[mostly single-threaded].
...
To internalize the potential issues & gains, let's say we're working on a 100k {abbr-loc} application - if compiling our app, excluding its dependencies, takes 5 minutes now, then by splitting it into 5 separate 20k LOC crates, we might reduce this time to as low as 1~2 minutes.
...
If you've already had the pleasure of working on a gigantic project, you might know the pain of https://xkcd.com/303/[waiting for the compiler to finish]: you've changed some error message, wanted to quickly see how it looks in the context and - ｏｈ ｇｏｄ - do you hear this sound? it's not a military aircraft, it's your computer's fans preparing for some hot & long code munching; it's daunting.
...
[NOTE]
====
One of the bigger - if not the biggest - workspace-based project I know is the almighty https://github.com/rust-lang/rust/tree/master/compiler[rustc].
====

Considering our toy project won't reach 10k LOC, why am I even talking about all those workspaces and crates?

https://www.youtube.com/watch?v=J2eud_tEIj8[I just think they're neat].

Also, I consider them to be a *good practice*: workspaces allow to introduce clear-cut boundaries between project's modules, and - when applied within reason - not only make the code cleaner, but _force it_ to be.
...
Simply because https://github.com/rust-lang/cargo/issues/8365[Cargo doesn't support `cargo new --workspace` yet].
...
Our `Cargo.toml` does not, as it's used to merely _group_ all the packages together without being a crate on its own; formally speaking, our `Cargo.toml` is a https://doc.rust-lang.org/cargo/reference/workspaces.html#virtual-manifest[virtual manifest].
====
...
We'll start *top-down*, with a structure modelling the entire network - it will provide sort of an *entry point* to our neural network crate:
...
image::/resources/learning-to-fly-pt2/coding-propagate-1.png[role=drawing]
...
\... with `()` being called https://doc.rust-lang.org/std/primitive.unit.html[unit value] (or *unit type*, depending on the context).

To answer the problem of _but i really don't know how this function should look like just yet_, Rust provides two macros: `todo!()`, and its older cousin - `unimplemented!()`.
...
image::/resources/learning-to-fly-pt2/coding-propagate-2.png[role=drawing]
...
image::/resources/learning-to-fly-pt2/coding-propagate-3.png[role=drawing]
...
image::/resources/learning-to-fly-pt2/coding-propagate-4.png[role=drawing, width=50%]
...
For instance, professional neural networks (for performance reasons) are usually implemented using https://towardsdatascience.com/diy-ai-an-old-school-matrix-nn-401a00021a55[matrices] - if we ever decided to rewrite our network to use matrices, then it wouldn't be a breaking change: `Network::propagate()`-s signature would remain the same and since users can't access `Layer` and `Neuron`, they wouldn't be able to notice these two structs being gone.
...
image::/resources/learning-to-fly-pt2/coding-propagate-5.png[role=drawing, title="Ecstasy of Saint Ferris (upon seeing idiomatic code), colorized"]

First of all, this is called a *rebinding* (or _shadowing_):
...
[.text-attached]
https://play.rust-lang.org/?version=stable&mode=debug&edition=2018&gist=945c2c2852ccbeb143ea569747038626[(playground link)]
...
fn bar(mut items: &Vec<usize>) {
    ...
    // But if the type remains immutable, you cannot modify the thing
    // itself:
    items.push(1234);
    //   ^^^^^ cannot borrow `*items` as mutable, as it is behind a
    //         `&` reference
}
----
...
Let's move on to neurons - a single neuron accepts many inputs and returns a _single_ output, so:
...
The way we wrote our algorithm is correct, but _inefficient_ - since we know how many output values we'll have, we can use this information to *preallocate* our vector:
...
[NOTE]
====
`Vec::with_capacity()` can be a great low-hanging fruit if your application allocates lots of _large_ vectors, but using it is not always worth the hassle; as usually when it comes to performance, a benchmark's your best friend.
====

Using `with_capacity()` might be awkward as times and so Rust, fulfilling its promise of zero-cost abstractions, allows to invoke it somewhat transparently:
...
Well, iterators contain a method called https://doc.rust-lang.org/stable/std/iter/trait.Iterator.html#method.size_hint[`Iterator::size_hint()`] - it returns an iterator's length (or `None`, if the length's not known). When you do `vec.iter()`, it simply creates an iterator that knows its length and then https://github.com/rust-lang/rust/blob/9775ffef2a4c3a36cadb58b72ea60cefb92c86ae/library/alloc/src/vec/spec_from_iter_nested.rs#L27[`.collect()`] uses that information to automatically create a vector that's large enough; no magic!

As you can see, even though our refactored code is technically _more_ complex (we're using an anonymous function and iterators, after all), it's at least equally performant and way more readable; writing optimized Rust code frequently cuts down to using high-level structures instead of trying to outsmart the compiler with fancy, hand-written loops.

Currently we've got nowhere else to go, but to complete `Neuron::propagate()` - as before, let's start with a crude, https://en.wikipedia.org/wiki/Superfund[superfund]-ish version:
...
. Given `inputs.len() < self.weights.len()`,
. Given `inputs.len() == self.weights.len()`,
. Given `inputs.len() > self.weights.len()`.
...
. We could change `Neuron::propagate()` to return an error message:
+
[source, rust]
----
...
----
+
\... or, using one of the crates I love the most - https://github.com/dtolnay/thiserror[thiserror]:
+
[source, rust]
----
...
----
+
. We could use the `assert_eq!()` / `panic!()` macros:
+
[source, rust]
----
fn propagate(&self, inputs: &[f32]) -> f32 {
...
. If this assertion ever fails, it means that our implementation is most likely wrong and there's nothing users could do on their side to mitigate it.
...
\... is https://doc.rust-lang.org/stable/std/primitive.f32.html#method.max[`f32::max()`] in disguise:
...
When instead of indexing, you use combinators such as `.zip()` or `.map()`, you make it _easier_ for the compiler to remove overzealous bound checks, making your code a bit faster.
...
[NOTE]
====
The `::<>` syntax used in the last line is called https://techblog.tonsser.com/posts/what-is-rusts-turbofish[turbofish] - it allows to provide explicit generic arguments when the compiler has troubles inferring them.
====
...
Our first approach for creating a constructor could be a plain, {abbr-no-op} function of:
...
image::/resources/learning-to-fly-pt2/coding-new-1.png[role=drawing, width=33%]

\... perhaps you'll notice that it's actually defined by *two* numbers: its _input_ and _output_ size; does it mean our single-field `LayerTopology` is wrong? Au contraire!
...
image::/resources/learning-to-fly-pt2/coding-new-2.png[role=drawing]
...
pub fn random(layers: Vec<LayerTopology>) -> Self {
    ...
    for i in 0..(layers.len() - 1) {
        let input_neurons = layers[layer_id].neurons;
        let output_neurons = layers[layer_id + 1].neurons;
...
As for the `for` loop - iterating by *adjacent* items is another pattern covered by the Rust's standard library, via function called https://doc.rust-lang.org/stable/std/primitive.slice.html#method.windows[`.windows()`]:
...
[NOTE]
====
If you know a thing or two about https://doc.rust-lang.org/book/ch18-03-pattern-syntax.html#destructuring-to-break-apart-values[destructuring], you might've thought of rewriting this loop even further:
...
Nightly Rust, having parts of *const generics* stabilized, offers a solution - a peachy function called https://doc.rust-lang.org/stable/std/primitive.slice.html#method.array_windows[`.array_windows()`]:
...
What's next, what's next... _checks notes_... ah, `Layer::random()`!
...
[NOTE]
====
`|_|`, also known as https://www.reddit.com/r/rustjerk/comments/8udbth/a_new_war_begins_choose_your_side/[toilet closure], is a function that accepts an argument it doesn't care about.
...
Also, the `_` itself is called a https://doc.rust-lang.org/reference/patterns.html#destructuring[placeholder] and it can be used in a few different contexts:
...
[.text-center.text-large.text-rainbow]
it's crate time!

Which crate should we choose? Well, let's see:

video::/resources/learning-to-fly-pt2/rand.webm[]

When it comes to PRNGs, https://crates.io/crates/rand[`rand`] is the *de facto standard*, https://rust-lang-nursery.github.io/rust-cookbook/algorithms/randomness.html[extremely versatile] crate that allows to generate not only pseudo-random numbers, but also other types such as strings.

In order to fetch `rand`, we have to add it to our neural network's `Cargo.toml`:
...
Certainly, `rng.gen_range(-1.0..=1.0)` predicts Dogecoin prices quite accurately - but is there a way we could ensure our _entire_ network works as intended?

[[testing]]
== Testing

A https://en.wikipedia.org/wiki/Pure_function[pure function] is a function whose given the same arguments, always returns the same value.
...
\... but that's a development counterpart of a https://en.wikipedia.org/wiki/False_friend[false friend] - though it's better than no test at all, it doesn't check much.

On the other hand, making `Neuron::random()` a pure-pure function seems... preposterous? What's the point of _randomizing_, if the outcome would always remain the same?
...
Side note: I've done it https://github.com/Patryk27/lxd-snapper/blob/9b096962122988abb3677a69ced244ea7cec2fa2/src/cmds/backup.rs#L159[in the past] and it's proven to be convenient.

Due diligence, for all the _purists_ out there: both `do_something()`-s and `random()`-s technically are _not_ pure functions as they all lack a property called https://en.wikipedia.org/wiki/Referential_transparency[referential transparency]. Though, if one's insistent, there's always:
...
Because the `rand` crate doesn't provide a predictable or seedable PRNG, we have to make use of another crate - I like `rand_chacha` (easy to remember, you probably already memorized it):
...
Notice the numbers are _different_ and that's alright - they are allowed to be different as long as each `cargo test` consistently works on the same *set* of numbers (and it does, because we used a PRNG with a constant seed).

Before moving on to another test, there's one touch we really should apply here - it stems from *floating-point inaccuracies*.

The type we're using, `f32`, models a 32-bit floating-point number that can represent values between `~1.2*10^-38` and `~3.4*10^38` - alas, it cannot represent _all_ of those numbers, just _some_.

For instance, with `f32` you cannot encode _exactly_ `0.15` - it'll always be off by a bit:
...
To avoid reinventing the wheel, I'll just drop this link: https://floating-point-gui.de[What Every Programmer Should Know About Floating-Point Arithmetic] - if you haven't read about floating-point numbers yet, I encourage you to give it a shot!
...
This is _the_ standard way to compare floats across all programming languages that implement https://en.wikipedia.org/wiki/IEEE_754[IEEE 754] (so, like, basically all the languages): instead of fishing for an *exact* result, you simply compare both numbers with some *margin of error* (also called *tolerance*).
...
\... or - which is the best approach - using a crate such as https://docs.rs/approx/0.4.0/approx/[approx]:
...
_If_ we had some *edge cases* to consider, say:
...
fn test() {
    ...
    approx::assert_relative_eq!(
        neuron.propagate(&[-100.0]),
        0,
    );

    // `1.5` and `-0.5` chosen by a fair dice roll:
    approx::assert_relative_eq!(
        neuron.propagate([1.5, -0.5]),
        (-0.3 * 1.5) + (0.8 * -0.5) + 0.5,
    );

    // We could've written `-0.35` right away, but showing the entire
...
In the meantime, feel free to checkout the *entire* source code at https://github.com/Patryk27/shorelark/tree/main/libs/neural-network[my GitHub repository].

=== What about https://github.com/rust-lang/rustfmt[rustfmt]?

This might come off like an advertisement, but let's give it a shot anyway:

Code formatting _is_ important - doesn't matter if your project's big or small, pass:[<a class="text-rainbow" href="https://github.com/rust-lang/rustfmt">rustfmt</a>] is the way to go!

=== What about https://pl.wikipedia.org/wiki/Test-driven_development[TDD]?

Test-driven development is a programming methodology in which you _first_ write tests and _only then_ start to work on the implementation; I use it more often than not, but while writing this article, I've decided that starting with implementation will be more educational than the adventure TDD would take us on.
...
It's because we could learn more this way - we _could've_ coded our network in 1/10th of its current size by using https://nalgebra.org/[nalgebra], we _could've_ used one of the http://www.arewelearningyet.com/neural-networks/[already-existing crates], but it's not the destination that matters, it's the journey.
