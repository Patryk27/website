...
#[derive(Debug, Parser)]
pub struct CompilePost;

impl CompilePost {
    pub fn run(
        self,
        env: &mut Env,
        mut nodes: Option<&mut Vec<Node>>,
    ) -> Result<()> {
        ...
        if let Err(err) = result {
            return match err.downcast() {
                Ok(msg) => {
                    MessageReporter::new(&mut env.stderr, &input)
                        .report(msg)?;
...
mod tests {
    ...
    fn test(case: &str) {
        ...
        let (result, stdout, mut stderr) =
            Env::mock(stdin, |env| CompilePost.run(env, Some(&mut nodes)));
